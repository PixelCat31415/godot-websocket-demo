...
static var _instance: WebSocketServer = null

static func get_instance() -> WebSocketServer:
	return _instance

func _ready() -> void:
	_instance = self
	assert(listen(PORT) == OK)
...
func _poll() -> void:
	...
	for p: PendingPeer in pending_peers:
		...
		elif (status == CONNECT_FAILED or
			  p.connect_time + handshake_timeout_msec < Time.get_ticks_msec()):
